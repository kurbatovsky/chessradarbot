...
def _is_user_due(setting, now_utc):
...
def _select_new_matching_tournaments(user_id, user_filters, tournaments):
    ...
    for tournament in tournaments:
        ...
        matched = matches_tournament(tournament, user_filters)
...
async def process_due_users():
    ...
    for setting in settings:
        ...
        save_results_cache(int(user_id), matched, user_filters)

        try:
            await send_notification_message(user_id, matched, user_filters)
            create_delivery_records(
                user_id=user_id,
